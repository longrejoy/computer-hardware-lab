...
{
	{
		...
		while ((HWREG(ui32Base + SSI_O_SR) & SSI_SR_TFE) == 0) //检查指定的SPI标志位设置与否:发送缓存空标志位
		{
			retry++;
			if(retry>200)return 0;
		}
		SSIDataPut(ui32Base,TxData); //通过外设SPIx发送一个数据
		retry=0;

		while ((HWREG(ui32Base + SSI_O_SR) & SSI_SR_RNE) == 0) //检查指定的SPI标志位设置与否:接受缓存非空标志位
		{
			retry++;
			if(retry>200)return 0;
		}
		SSIDataGet(ui32Base,&RxData);
		return (RxData & 0xff); //返回通过SPIx最近接收的数据
	}
}
//
...
//! - SSI0Rx  - PA4
//! - SSI0Tx  - PA5
static void ENC28J60_SPI0_Init(void)
{
	//
    ...
    // freescale SPI mode, master mode, 9MHz SSI frequency, and 8-bit data.
...
void ENC28J60_Reset(void)
{
	...
	ENC28J60_SPI0_Init();	//SPI0初始化
	ENC28J60_RST_RESET;			//复位ENC28J60
	SysCtlDelay(133333);    //延时10ms
...
u8 ENC28J60_Read_Op(u8 op,u8 addr)
{
	...
	SPI_ReadWriteByte(SSI0_BASE,dat);
	dat=SPI_ReadWriteByte(SSI0_BASE,0xFF);
	//如果是读取MAC/MII寄存器,则第二次读到的数据才是正确的,见手册29页
 	if(addr&0x80)dat=SPI_ReadWriteByte(SSI0_BASE,0xFF);
	ENC28J60_CS_SET;
	return dat;
}
//读取ENC28J60寄存器(带操作码) 
...
void ENC28J60_Write_Op(u8 op,u8 addr,u8 data)
{
	...
	dat=op|(addr&ADDR_MASK);
	SPI_ReadWriteByte(SSI0_BASE,dat);
	SPI_ReadWriteByte(SSI0_BASE,data);
	ENC28J60_CS_SET;
}
//读取ENC28J60接收缓存数据
//len:要读取的数据长度
...
{
	ENC28J60_CS_RESET;
	SPI_ReadWriteByte(SSI0_BASE,ENC28J60_READ_BUF_MEM);
	while(len)
	{
		len--;			  
		*data=(u8)SPI_ReadWriteByte(SSI0_BASE,0);
		data++;
	}
	*data='\0';
	ENC28J60_CS_SET;
...
{
	ENC28J60_CS_RESET;
	SPI_ReadWriteByte(SSI0_BASE,ENC28J60_WRITE_BUF_MEM);
	while(len)
	{
		len--;
		SPI_ReadWriteByte(SSI0_BASE,*data);
